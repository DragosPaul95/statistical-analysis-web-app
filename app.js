    ...
    try {
        fiber(function () {
            ...
            var columnTotals = resultMatrix.map(function (row, i) {
                return sum(resultMatrix.map(function (row) {
                    return row[i];
                }));
            });
            columnTotals.pop();
...
app.post('/aslData/:questID', function (req, res) {
    try {
        fiber(function () {
            var queryString = 'SELECT up.* FROM users_profile up JOIN answers a ON a.answer_user_id = up.user_id JOIN survey_questions sq ON sq.question_id = a.answer_question_id JOIN surveys s ON s.survey_id = sq.question_survey_id WHERE s.survey_id = 23 GROUP BY up.user_id';
            var answeredUserData = await(db.query(queryString, req.params.questID, defer()));
            res.send(answeredUserData);
        });
    } catch (e) {
        throw e;
    }
...
app.post('/user/:userID', function (req, res) {
    try {
        fiber(function () {
            var checkIfExistsQuery = await(db.query('SELECT * FROM users_profile WHERE user_id = ?', req.params.userID, defer()));
            var query;
            if (checkIfExistsQuery && checkIfExistsQuery.length > 0) {
                //user already has profile, we update
