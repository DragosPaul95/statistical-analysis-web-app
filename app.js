    ...
    try {
        fiber(function () {
            ...
            for (var key in geoMeanObj) {
                ...
                geoMeanArr.push({
                    key: key,
                    mean: geometricMeanFunc(splitVals).toFixed(4)
                })
            }
    ...
    try {
        fiber(function () {
            ...
            for (var i = 0; i < x.length; i++) {
                ...
                sumYsq += Math.pow(parseInt(y[i].answer), 2);
            ...
            var dxdy = sumXY - ((sumX*sumY)/x.length);
            var dxSq = sumXsq - ((Math.pow(sumX,2))/x.length);
            var dySq = sumYsq - ((Math.pow(sumY,2))/y.length);
            var r = dxdy/Math.sqrt(dxSq*dySq);
